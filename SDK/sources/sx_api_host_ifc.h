...
/**
 * Set filter for registered traps (STP, LACP etc.) or events (Port up /down, temperature event etc.)
 * in the driver on the provided user channel.
 * Filters can be set before or after registration of the user channel to receive traps.
 * Traps that match filter key shall not be forwarded to the user channel. The filter takes priority on
 * traps registration, regardless of the order of API calls.
 * In order to add a filter use SX_ACCESS_CMD_ADD, In order to delete a specific filter use command
 * SX_ACCESS_CMD_DELETE and specify filter attributes.
 ...
 * @return SX_STATUS_MEMORY_ERROR error handling memory
 * @return SX_STATUS_NOT_FOUND object to delete was not found
 */
...
/**
 * This function gets a list of filter entries (filter key and user channel) according to the swid
 * and  trap ID.
 * The following use case scenarios apply with different input parameters.
 * 1) SX_ACCESS_CMD_GET
 *    When *filter_entry_cnt_p is not set to 0, this function returns a list of filter entries.
 *    The returned filter_entry_cnt_p may be less or equal to the requested filter_entry_cnt_p.
 *    When *filter_entry_cnt_p is set to 0, this function returns the total number of filter entries
 *    configured for the specified trap ID.
 *    The input filter entry is ignored in this case.
 *
 * 2) SX_ACCESS_CMD_GET_FIRST
 *    In this case the API will return the first set of filter entries.
 *    The returned filter_entry_cnt may be less or equal to the requested filter_entry_cnt.
 *    The input filter entry is ignored in this case.
 *
 * 3) SX_ACCESS_CMD_GETNEXT
 *    In this case the API will return the next set of filter entries starting from
 ...
 * @param[in,out] filter_entry_cnt_p     - [in]: number of filter entries requested.
 *                                       - [out]: number of filter entries returned.
 *
 * @return SX_STATUS_SUCCESS if operation completes successfully
