 ...
 * @return SX_STATUS_CMD_UNSUPPORTED if provided cmd is unsupported
 * @return SX_STATUS_CMD_UNPERMITTED - is ISSU is in progress
 ...
 * @return SX_STATUS_NO_MEMORY - if system has no more available free memory
 */
 ...
 * Once this API is called sx_api_lag_hash_flow_params_set is disabled.
 *
 * Command SET replaces existing hash parameters, field enables and fields with new values.
...
/**
 * This function gets the given port's LAG hash function configuration parameters.
 ...
 * @return SX_STATUS_PARAM_ERROR if any of the cnt parameters has an invalid value.
 * @return SX_STATUS_ERROR general error.
 * @return SX_STATUS_ENTRY_NOT_FOUND if lag is not found
 */
 ...
 *  If so, the redirected LAG logical ID is returned.
 *  Supported devices: Spectrum, Spectrum2, Spectrum3.
 *
 * @param[in] handle                     - SX-API handle.
 * @param[in] lag_log_port               - LAG logical ID.
 ...
 *                                         points to when the LAG is redirected.
 *
 * @return SX_STATUS_SUCCESS if operation completes successfully
 * @return SX_STATUS_INVALID_HANDLE if a NULL handle is received
 * @return SX_STATUS_PARAM_NULL if a parameter is NULL
...
/**
 *  This function returns all the LAGs that are redirected to the given LAG.
 *  Calling this API with lag_log_port_list_p=NULL will return the
 *  number of LAGs redirected to lag_port.
 *  Supported devices: Spectrum, Spectrum2, Spectrum3.
 *
 * @param[in] handle                    - SX-API handle
 * @param[in] lag_log_port              - LAG logical ID
...
/**
 *  This function sets/deletes a fine grained LAG distribution list.
